...
import { useState, useEffect, use as reactUse, useRef } from "react";
...
export default function EditTournamentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await update(id, { title, description: description || undefined, gameId, format, mode, status, maxPlayers, entryFee: entryFee || undefined, currency, prizePool: prizePool || undefined, bannerUrl: finalBannerUrl || undefined });
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      ...
      <form onSubmit={handleSubmit} className="glass-card card-hover space-y-4 rounded-2xl p-6">
        <Field label="Título"><input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required className={inputCls} disabled={loading} /></Field>
        <Field label="Descrição"><textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className={inputCls} disabled={loading} /></Field>
        <Field label="Jogo"><select value={gameId} onChange={(e) => setGameId(e.target.value)} required className={selectCls} disabled={loading}>{games.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}</select></Field>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <Field label="Estado"><select value={status} onChange={(e) => setStatus(e.target.value)} className={selectCls} disabled={loading}><option value="DRAFT">Rascunho</option><option value="OPEN">Aberto</option><option value="ONGOING">Em curso</option><option value="FINISHED">Terminado</option><option value="CANCELED">Cancelado</option></select></Field>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <Field label="Jogadores máx."><input type="number" value={maxPlayers} onChange={(e) => setMaxPlayers(Number(e.target.value))} min={2} className={inputCls} disabled={loading} /></Field>
        ...
        <Field label="Banner do torneio">
          <div className="space-y-2">
            <div className="flex items-end gap-3">
              <div className="flex-1">
                <input type="url" value={bannerUrl} onChange={(e) => setBannerUrl(e.target.value)} className={inputCls} disabled={loading} />
              </div>
              <button
                type="button"
            ...
            {bannerPreview ? (
              ...
              <img src={bannerUrl} alt="Banner atual" className="h-32 w-full rounded-lg border border-violet-500/20 object-cover" />
            ) : null}
            <p className="text-xs text-zinc-400">A imagem será enviada ao Cloudinary quando clicar em "Guardar"</p>
          </div>
        </Field>
        
        <div className="flex gap-3 pt-2">
          <Button type="submit" disabled={loading} className="flex-1 rounded-lg py-2.5">{loading ? "A guardar…" : "Guardar"}</Button>
          <Link href={`/tournaments/${id}`}><Button variant="secondary" className="rounded-lg" disabled={loading}>Cancelar</Button></Link>
        </div>
      </form>
    </div>
  );
