...
export default function EditGamePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await update(id, { name, slug, coverUrl: finalCoverUrl || undefined });
      router.push("/games");
    } catch (err) { 
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-8">
      ...
      <form onSubmit={handleSubmit} className="glass-card card-hover space-y-4 rounded-2xl p-6">
        <Field label="Nome"><input type="text" value={name} onChange={(e) => setName(e.target.value)} required className={inputCls} disabled={submitting} /></Field>
        <Field label="Slug"><input type="text" value={slug} onChange={(e) => setSlug(e.target.value)} required className={inputCls} disabled={submitting} /></Field>
        
        <Field label="Capa do jogo">
          <div className="space-y-2">
            <div className="flex items-end gap-3">
              <div className="flex-1">
                <input type="url" value={coverUrl} onChange={(e) => setCoverUrl(e.target.value)} className={inputCls} disabled={submitting} />
              </div>
              <button
                type="button"
            ...
            {coverPreview ? (
              ...
              <img src={coverUrl} alt="Capa atual" className="h-32 w-full rounded-lg border border-violet-500/20 object-cover" />
            ) : null}
        ...
        <div className="flex gap-3 pt-2">
          <Button type="submit" disabled={submitting || loading} className="flex-1 rounded-lg py-2.5">{submitting ? "A guardar…" : "Guardar"}</Button>
